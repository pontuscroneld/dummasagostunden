...
class ReadView: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if(story == "sorry"){
            thestory.text = "Hej \(storywords[0]), det är jag, \(storywords[1]). Jag vet att jag sårade dig igår när slog dig i \(storywords[2]) och sa att du hade \(storywords[3]) \(storywords[4]). Jag menade det inte och sanningen är att jag tycker att du är \(storywords[5]) som ett \(storywords[6]). Och det vill jag att du ska veta. Egentligen handlar allting om mig själv, att du kan så mycket mer om \(storywords[7]) än jag och om mina egna förväntningar på mig själv. Jag har alltid drömt om att rida \(storywords[8]) i \(storywords[9]) och att du redan har gjort det kan få mig att känna mig \(storywords[10]). Jag vill vara din \(storywords[11]) igen. Kan vi ses imorgon vid \(storywords[12]) på \(storywords[13]) där vi möttes första gången? Där vi sjöng på \(storywords[14]) och dansade som \(storywords[15]). Jag kommer stå där i en \(storywords[16]) \(storywords[17]) och vänta på dig. Förlåt, Din käre \(storywords[11])."
        }
        ...
        if(story == "home"){
            thestory.text = "Greven \(storywords[0]) bor i landet \(storywords[1]). Har du aldrig hört talas om det? Det ligger bara \(storywords[2])s färd ifrån \(storywords[3]), som jag förmodar att du känner till. Greven vann alldeles nyligen \(storywords[4]) kronor på lotto och för dom pengarna ska han bygga ett nytt slott. Han har planerat allt in i minsta detalj. Föreställ er en \(storywords[5]) port som leder in i byggnaden, omringat av tusen \(storywords[6]). Hallen har ett golv av marmor och väggarna är gjorda av \(storywords[7]). På den \(storywords[8]) våningen ligger hans sovrum med utsikt över den \(storywords[9]) skogen. Där sägs finnas både vilda \(storywords[10]) och en \(storywords[11]) som ger en kraften att förstå \(storywords[12]) och all makt som det innebär. Greven är en vänlig man, men han längtar efter \(storywords[13]) vänskap. Har han tur kanske han träffar han dig imorgon på \(storywords[14]) när han handlar något att \(storywords[15]) med i sitt nya hem. Så kan du få följa med och se grevens nya, \(storywords[16]) hem."
        }
